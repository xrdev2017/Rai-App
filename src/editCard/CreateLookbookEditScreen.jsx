...
const CreateOutfitEditScreen = () => {
  ...
  const { updateItemLookbook, updateOutfitLookbook } = getValues();

  const data = updateItemLookbook ? updateItemLookbook : updateOutfitLookbook;

  // console.log("LINE AT 510", data, updateItemLookbook);
  ...
  useEffect(() => {
    if (data) {
      setValue("season", data.season?.[0] || "");
      setValue("outfitStyle", data.style?.[0] || "");
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      updateItemLookbook
        ? await deleteItem(data._id).unwrap()
        : await deleteOutfit(data._id).unwrap();
  ...
  const handleSave = async () => {
    try {
      ...
      let res;
      updateItemLookbook
        ? (res = await updateAddItem({
            id: data._id,
            data: formData,
          }).unwrap())
        : (res = await updateCreateOutfit({
            id: data._id,
            data: formData,
          }).unwrap());

      // console.log("LINHE AT 1021", res);
        ...
        >
          ...
          <View className="flex-row gap-2 items-center justify-between py-2">
            <Pressable
              ...
              onPress={handleSave}
              disabled={updateCreateOutfitLoading || updateAddItemLoading}
            >
              <Text className="text-white font-SemiBold text-[16px]">
                {updateCreateOutfitLoading || updateAddItemLoading
                  ? "Saving..."
                  : "Save"}
                {/* Save */}
              </Text>
          ...
          <DeleteAccountModal
            ...
            loading={deleteOutfitLoading || deleteItemLoading}
          />
          <ViewImageModal
